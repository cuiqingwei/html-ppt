...
- 字体：`Fira Code`（英文/符号/代码）+ `LXGW WenKai`（中文正文），均从 Google Fonts 引入
- 全局背景网格：使用 `var(--grid-color)` 的水平+垂直线条，间距 44px，`position: fixed`
- 进度条：顶部固定，渐变 `var(--accent) → var(--accent3)`，带发光阴影

### 字体系统

**原则：所有中文使用 `LXGW WenKai`，所有英文及 ASCII 符号使用 `Fira Code`，通过 `@font-face unicode-range` 精确切割，无需依赖 fallback 猜测。**

**Google Fonts 引入（`<head>` 内）**

```html
<link rel="preconnect" href="https://fonts.googleapis.com">
<link href="https://fonts.googleapis.com/css2?family=LXGW+WenKai:wght@300;400;700&family=Fira+Code:wght@400;500;600&display=swap" rel="stylesheet">
```

**双字体虚拟字体族（CSS `@font-face`）**

在 `<style>` 内声明两个虚拟字体族 `SiteFont`（正文）和 `SiteFontMono`（代码/标签），每个字体族分别通过两条 `@font-face` 规则切割 unicode-range：
...
@font-face {
  ...
  font-weight: 100 900;
  unicode-range:
    U+0020-007E, U+00A0-02FF, U+0300-036F, U+0370-03FF,
    U+2000-206F, U+2070-209F, U+20A0-22FF, U+2300-23FF,
    U+25A0-26FF, U+FB00-FB06;
}
/* LXGW WenKai 负责全部 CJK 字符 */
@font-face {
  font-family: 'SiteFont';
  ...
  font-weight: 100 900;
  unicode-range:
    U+2E80-2EFF, U+2F00-2FDF, U+3000-303F, U+3040-30FF,
    U+3100-318F, U+3190-31EF, U+31F0-33FF, U+3400-4DBF,
    U+4E00-9FFF, U+A000-A4CF, U+F900-FAFF, U+FE10-FE6F,
    U+FF00-FFEF, U+20000-2CEAF;
}

/* 全局应用：所有元素默认使用 SiteFont */
*, *::before, *::after {
  font-family: 'SiteFont', 'LXGW WenKai', 'Fira Code', sans-serif;
}
```
...
| 场景 | font-family 值 |
|------|---------------|
| `body` 正文、标题、卡片 | `'SiteFont', 'LXGW WenKai', serif` |
...
- 单 HTML 文件，所有 CSS/JS 内联，无外部依赖（除 Google Fonts）
...
*模板版本：v1.2 · 2025 · 新增双主题切换 + T 快捷键 + 双字体系统（Fira Code / LXGW WenKai）*
